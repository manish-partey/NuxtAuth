...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Only super_admin and platform_admin can delete document types
    if (!['super_admin', 'platform_admin'].includes(user.role)) {
      throw createError({
        statusCode: 403,
        statusMessage: 'Insufficient permissions to delete document types'
      });
    }
    ...
    console.log(`[ADMIN] Document type deleted: ${documentType.name} (${documentType.key}) by ${user.email}`);
